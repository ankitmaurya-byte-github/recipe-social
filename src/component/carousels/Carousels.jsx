...
import { Timestamp, arrayUnion, collection, getDocs,getDoc, onSnapshot, setDoc, updateDoc } from 'firebase/firestore'
...
function Carousels({loading,data}) {
    ...
    //     const getChats = () => {
    //       const unsub = onSnapshot(doc(db,"likes",user.uid), (doc) => {
    //         // console.log(doc);
    //       });
    ...
    const handelclick=async(e,item)=>{
        if(e.target.className=="material-symbols-rounded " || e.target.className=="material-symbols-rounded red" || e.target.className=='cuisineType' || e.target.className=='label'){
            return
        }
        try{
  ...
  return (
    <div className='carousel'>
        <span onClick={()=>handelscroll("left")} class="material-symbols-rounded">
arrow_circle_left
</span>
<span onClick={()=>handelscroll("right")} class="material-symbols-rounded">
arrow_circle_right
</span>
        <div ref={ref} className="carouselsItems">
            {!loading && data?.map((item,index)=>{
                ...
                return (
                    <div key={index} onClick={(e)=>handelclick(e,item.recipe)} className="carouselItem">
                        
                        <img src={object?.image} alt="" />
                        <p className='label'>{object?.label}</p>
                        <p className='cuisineType'>{object?.cuisineType[0]}</p>
                        <span onClick={()=>handellike(item.recipe)} class={`material-symbols-rounded ${isliked(item.recipe)?'red':''}`}>
favorite
</span>
