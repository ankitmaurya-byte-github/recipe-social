...
useEffect(()=>{
  ...
  const handelclick=async(e,item)=>{
    if(e.target.className=="material-symbols-rounded " || e.target.className=="material-symbols-rounded black" || e.target.className=='cuisineType' || e.target.className=='label'){
        return
    }
    try{
        ...
        try{
          ...
          const q=query(collection(db,'users'),where("displayName","==",username))
            ...
            const result=await getDoc(doc(db,'likes',data))
                if(result.exists()){
                    const uriData=await Promise.all(result.data().uri.map(async(item)=>{
...
contact_page
    ...
    <div className='favourities'>
      <div className="name">Favourities<span class="material-symbols-rounded">
heart_check
</span></div>
      ...
      {likedDish.map((item,index)=>{
        ...
        if(!item)return <p>somthing wrong</p>
                ...
                return (
                    <div key={index} onClick={(e)=>handelclick(e,object)} className="carouselItem">
                        
                        <img src={object?.image} alt="" />
                        <p className='label'>{object?.label}</p>
                        <p className='cuisineType'>{object?.cuisineType[0]}</p>
                        {user?.uid && <span onClick={()=>handellike(object)} class={`material-symbols-rounded black`}>
delete
</span>}
