...
function Detail() {
    ...
    useEffect(()=>{
        const unsub=async()=>{
          const q=query(collection(db,'users'),where("displayName","==",username))
                ...
                dispatch(changedata(result.dishDetail))
        }
       !user && unsub()
      },[user])
    useEffect(()=>{
        ...
        const unsub=async()=>{
            try{
                ...
                dispatch(changedata(res.data().dishDetail))
            }catch(err){
                console.log(err);
            }
        }
    ...
    return (
        <>
        {user?.uid && <Header/>}
        { !user?.uid && <div className={`detailhead ${show}`}>
            <div className="navigate">
            <div onClick={()=>navigate(`/favourites/${username}`)} className="navigatefavourities">
            Favourites <span class="material-symbols-rounded">
heart_check
</span>
            </div>
            <div onClick={()=>navigate(`/profile/${username}`)} className="navigateprofile">
          Profile <span class="material-symbols-rounded">
contact_page
</span>
            </div>
        ...
        <div className="container ">
            ...
            <div className="details">
                <div className="info">
                    ...
                    <div className="types">
                        <div className="mealtype">
                            <h3>mealType</h3>
                            <div className="test">
                               {data.mealType?.map((type,index) => <p key={index}>{type}</p>)} 
                            </div>
                        ...
                        <div className="cuisinetype">
                            <h3>cuisineType</h3>
                            <div className="test">
                               {data.cuisineType?.map((type,index) => <p key={index}>{type}</p>)} 
                            </div>
                        </div>
                        <div className="dishtype">
                            <h3>dishType</h3>
                            <div className="test">
                               {data.dishType?.map((type,index) => <p key={index}>{type}</p>)} 
                            </div>
                        </div>
                    </div>
                </div>
                <div className="content">
                    ...
                    <div onClick={()=>settotalnutrients(!totalnutrients)} className={`totalnutrients ${totalnutrients?'strech':''}`}>
                        <h2>Total-nutrients</h2><span class="material-symbols-rounded">
lists
</span>
                    ...
                    <div onClick={()=>sethealthlabel(!healthlabel)}  className={`healthlabel ${healthlabel?'strech':''}`}>
                        <h2>Health-label</h2><span class="material-symbols-rounded">
lists
</span>
                        {healthlabel && data?.healthLabels?.map(health =>{
                            return <p>{health}</p>
                        })}
                    </div>
                    <div className="ingredientsline">
                        ...
                        {data?.ingredientLines?.map((line,index)=>{
                          return <div className='lineitems'> <span class="material-symbols-rounded">
                          mediation
                          </span> <p key={index}>{line}</p> <br /></div>
                        })}
                        </div>
                    </div>
                </div>
            ...
            <div className="recipe">
                <div className="ingredients">
                    ...
                    {data?.ingredients?.map((item,itemIndex) => (
                      <div className={`items ${shouldopen(itemIndex)?'':'shrink'}`} key={item.id}>
                        { shouldopen(itemIndex)? Object.entries(item).map(([key, value], index) => (
                          <div className='item'>
                          {index==0 && <span onClick={()=>handelshrink(itemIndex)}  class="material-symbols-rounded">
unfold_more
</span>} 
                         <h4>{key}:</h4>
                            <p>{value}</p>
                        ...
                        )):<span onClick={()=>handelshrink(itemIndex)}  class="material-symbols-rounded">
                        unfold_more
                        </span> }
                        <hr className='long' />
                      </div>
                ...
                <div className="source">
                    <h2>Source : </h2><p><Link to={data.url}>{data.source}</Link></p>
                </div>
